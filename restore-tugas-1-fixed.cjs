...
async function restore() {
  const targetClasses = ["X1", "X3", "X4", "X5", "X6", "X7", "X8", "X9"];
  const tugas1Data = {
  ...
  console.log("Restoring Tugas 1 for classes:", targetClasses.join(", "));
  await setDoc(doc(db, "assignments", "TGS-1785884777808"), tugas1Data);
  console.log("Successfully restored Tugas 1 to Firestore assignments collection!");
  
  const snap = await getDoc(doc(db, "assignments", "TGS-1785884777808"));
  console.log("Verified restored doc:", snap.exists(), snap.data().title, "| Kelas:", snap.data().kelas);
  process.exit(0);
}
